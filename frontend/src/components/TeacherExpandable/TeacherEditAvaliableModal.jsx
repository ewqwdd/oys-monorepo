...
export default function TeacherEditAvaliableModal({ open, setOpen, avaliable }) {
  ...
  const onFinish = (values) => {
    ...
    const timeFrom = values.time?.[0] ? values.time[0].format("HH:mm") : undefined;
    const timeTo = values.time?.[1] ? values.time[1].format("HH:mm") : undefined;
    api
      .put("/crm/avaliable", { ...values, timeFrom, timeTo, date: values.date ? values.date.format("YYYY.MM.DD") : undefined, teacher: avaliable.teacher, _id: avaliable._id })
      .then(res => {
        dispatch(commonActions.editAvaliable({ teacherId: avaliable.teacher, avaliable: res.data }));
        close();
      })
      .catch((err) => {
  ...
  const onDelete = () => {
    api.delete(`/crm/avaliable/${avaliable._id}`)
    .then(() => {
      dispatch(commonActions.deleteAvalialbe({ teacherId: avaliable.teacher, avaliableId: avaliable._id }));
      close();
    })
    .catch((err) => {
      messageApi.error(err?.response?.data?.message || "Помилка");
