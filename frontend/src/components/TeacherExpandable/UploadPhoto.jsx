...
export default function UploadPhoto({ data, setData }) {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      const response = await api.post(`/crm/teachers/image/${data._id}`, formData, {
        headers: {
  ...
  return (
    ...
    {contextHolder}
      ...
      {data.photo && <Image width={200} height={200} src={photo ?? data.photo} alt="Uploaded"  />}
      {data.photo && <Button type="dashed" color="danger" onClick={() => setData((prev) => ({ ...prev, photo: "" }))}>Видалити</Button>}
    </Flex>
  );
}
