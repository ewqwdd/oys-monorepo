  ...
  reducers: {
    ...
    editAvaliable: (state, action) => {
      if (state.user?._id === action.payload.teacherId) {
        const avaliable = state.user.avaliable.find(
          (avaliable) => avaliable._id === action.payload.avaliable._id,
        );
      ...
      const avaliable = teacher.avaliable.find(
        (avaliable) => avaliable._id === action.payload.avaliable._id,
      );
      Object.assign(avaliable, action.payload.avaliable);
    },
    ...
    addMeet: (state, action) => {
      const findIndex = state.meets?.findIndex(
        (meet) => meet._id === action.payload._id,
      );
      if (findIndex !== -1) {
    ...
    deleteClients: (state, action) => {
      state.clients = state.clients.filter(
        (client) => !action.payload.includes(client._id),
      );
    },
    deleteClientFromMeet: (state, action) => {
      const meet = state.meets.find(
        (meet) => meet._id === action.payload.meetId,
      );
      meet.clients = meet.clients.filter(
        (client) => client._id !== action.payload.clientId,
      );
    },
  },
});
